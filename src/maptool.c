         ...
         if (fs<=tmp) for (;; fs++) {
            ...
            */
            nn = libsrtm_getsrtmlong((double)(pos.lat+kf*osic_sin(nv)),
                ...
                &attr, 0);
            if ((uint32_t)attr==2UL) nn = nn+treealt(nn, trees);
   ...
   if (phtab) {
      ...
      while (phtab) {
         ...
         maptool_vector(image, phtab->tx, phtab->ty, xsh, ysh,
                ...
                0.0f); /* headroom line */
         maptool_waypoint(image, xsh, ysh, 1.5f,
                (int32_t)((uint32_t)red*3UL),
                (int32_t)((uint32_t)green*3UL),
                (int32_t)((uint32_t)blue*3UL));
   ...
   if (y<=tmp) for (;; y++) {
      ...
      if (x<=tmp0) for (;; x++) {
         if (colnr==0UL) {
            ...
            c += image->Adr[(x-1UL)*image->Len0+y].r;
            c += image->Adr[(x)*image->Len0+(y-1UL)].r;
            image->Adr[(x)*image->Len0+y].b = (uint16_t)((uint32_t)c/4UL)
                ;
         }
         else {
   ...
   if (y<=tmp) for (;; y++) {
      ...
      if (x<=tmp0) for (;; x++) {
         { /* with */
            ...
            if (colnr==0UL) {
               if ((uint32_t)anonym->b>(uint32_t)anonym->r) {
                  anonym->r = anonym->b;
               }
            }
            else if ((uint32_t)anonym->b>(uint32_t)anonym->g) {
               anonym->g = anonym->b;
            }
         }
         if (x==tmp0) break;
...
{
   if (colnr==0UL) {
      if ((uint32_t)bri>(uint32_t)p->r) p->r = bri;
   }
   else if ((uint32_t)bri>(uint32_t)p->g) p->g = bri;
} /* end setcol() */
      ...
      if (((frame==0UL && ytx>0.0f || frame==2UL && ytx<(float)
         ...
         if (oodist>1.E-6f) {
            ...
            do {
               ...
               if (hgnd<30000.0f) {
                  /* srtm valid */
                  if ((uint32_t)attr==2UL) {
                     hgnd = hgnd+treealt(hgnd, trees);
                  }
   ...
   do {
      ...
      */
      ...
      /*WrFixed(dist, 1,15); WrFixed(alt, 1,15); WrFixed(h, 1,15);
                WrStr(" =d alt h"); */
      if (h0<30000.0f) {
         if ((uint32_t)attr==2UL) *h = h0+treealt(h0, trees);
         else *h = h0;
         sp = *alt-*h;
         if (sp>0.0f) {
            qual = sp*0.25f;
...
/* lat/long + locator string of pos */
{
   ...
   if (nn<30000L) {
      ...
      if ((uint32_t)attr==3UL) aprsstr_Append(s, s_len, "W", 2ul);
      else if ((uint32_t)attr==2UL) aprsstr_Append(s, s_len, "T", 2ul);
      else if ((uint32_t)attr==1UL) aprsstr_Append(s, s_len, "U", 2ul);
   }
                ...
                char dryrun)
{
   ...
   if (((uint8_t)ch>=' ' && x0r>=0.0f) && y0r>=0.0f) {
      ...
      { /* with */
         ...
         *inc0 = (int32_t)((uint32_t)anonym->width+1UL);
      }
   }
} /* end drawchar() */
...
extern uint32_t maptool_charwidth(char ch)
{
   ...
   if ((uint8_t)ch>=' ') {
      cn = (uint32_t)(uint8_t)ch-32UL;
      if (cn>98UL) cn = 0UL;
      return (uint32_t)font[cn].width+1UL;
   }
   return 0UL;
} /* end charwidth() */
                ...
                uint32_t bri)
{
   ...
   uint32_t r;
   uint32_t tmp;
   switch ((uint32_t)area.color&7UL) {
   case 0UL: /* we have no black so make gray */
      r = 128UL;
      g = 128UL;
      b = 128UL;
      break;
...
static void loadsym(int32_t defy, char msg[], uint32_t msg_len)
{
   ...
   if (res>=0L) {
      ...
      if (insize<=48UL) {
         ...
         if ((uint32_t)rows[0U][0U].alpha>0UL && (uint32_t)
                rows[0U][0U].g8>128UL) alphax = 0UL;
               ...
               if (x<=tmp0) for (;; x++) {
                  { /* with */
                     ...
                     if (alphax==0UL) {
                        /* use white as alpha */
                        c = 0U;
                        if (((uint32_t)anonym->r8<230UL || (uint32_t)
                anonym->g8<230UL) || (uint32_t)anonym->b8<230UL) {
                           c = 255U;
                        }
                     }
   ...
   for (c = 0UL; c<=98UL; c++) {
      { /* with */
         ...
         if (y<=tmp) for (;; y++) {
            ...
            x = 0UL;
            if (x<=tmp0) for (;; x++) {
               if ((uint32_t)anonym->char0[y][x]>=90UL) {
                  /* 128 */
                  if (c) {
...
static void wr(int32_t fd, uint32_t * len, char b[32768],
                uint16_t c)
{
   if ((uint32_t)c<=1023UL) b[*len] = gammatab[c];
   else b[*len] = '\377';
   ++*len;
   if (*len>32767UL) {
...
{
   if ((uint32_t)c<=1023UL) return gammatab[c];
   else return 255U;
   return 0;
} /* end pngc() */
