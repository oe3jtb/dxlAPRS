...
/*
WrStrLn("");
FOR i:=0 TO HIGH(atab) DO WrFixed(atab[i], 2,8) END;
WrStrLn("");
*/
} /* end initafir() */
...
, DFM, IMET, M10, M20, MRZ, MEISEI", 84ul);
               osi_WrStrLn("Sonde Demodulator Ver:1.37m", 28ul);
               osi_WrStrLn("send raw Frames via UDP to \'sondemod\' decoder a\
nd/or watch only with -v", 72ul);
               osi_WrStrLn("more demodulators may send to same \'sondemod\'",
                 46ul);
               osi_WrStrLn("Stereo used for 2 Rx for 2 Sondes or 1 Sonde with\
 Antenna-Diversity or use \"sdrtst\"", 84ul);
...
default is 2", 62ul);
               osi_WrStrLn(" -t             send always TOA data to find posi\
tionless rs42, else switched on on demand", 91ul);
                ...
                int32_t ch, char withtoa)
{
   ...
   if (!nosendsdr) {
      ...
      if (withtoa) {
         b[p] = 't';
         ++p;
         b[p] = (char)(chan[ch].r41.toaus/16777216UL);
         ++p;
         b[p] = (char)(chan[ch].r41.toaus/65536UL&255UL);
         ++p;
         b[p] = (char)(chan[ch].r41.toaus/256UL&255UL);
         ++p;
         b[p] = (char)(chan[ch].r41.toaus&255UL);
         ++p;
      }
      t = osic_time();
...
static void decodeframe92(uint32_t m)
{
   ...
   { /* with */
      ...
      flen = 240UL;
      sdrparm(chan[m].r92.rxbuf, 301ul, &flen, (int32_t)m, 0);
...
static void sendrs41(uint32_t m)
{
   ...
   { /* with */
      ...
      sdrparm(anonym->r41.rxbuf, 560ul, &flen, (int32_t)m,
                testencr || encryptime+10UL>osic_time());
...
static uint32_t rs41date(const char f[], uint32_t f_len,
                uint32_t p)
{
   char s[21];
   uint32_t n;
   strncpy(s,"....-..-.. 00:00:00",21u);
   n = (uint32_t)(uint8_t)f[p]+256UL*(uint32_t)(uint8_t)f[p+1UL];
   s[0U] = (char)(n/1000UL+48UL);
   s[1U] = (char)((n/100UL)%10UL+48UL);
   s[2U] = (char)((n/10UL)%10UL+48UL);
   s[3U] = (char)(n%10UL+48UL);
   n = (uint32_t)(uint8_t)f[p+2UL];
   s[5U] = (char)(n/10UL+48UL);
   s[6U] = (char)(n%10UL+48UL);
   n = (uint32_t)(uint8_t)f[p+3UL];
   s[8U] = (char)(n/10UL+48UL);
   s[9U] = (char)(n%10UL+48UL);
   if (aprsstr_StrToTime(s, 21ul, &n)) {
      n += (uint32_t)(uint8_t)f[p+4UL]*3600UL;
      n += (uint32_t)(uint8_t)f[p+5UL]*60UL;
      n += (uint32_t)(uint8_t)f[p+6UL];
   }
   else n = 0UL;
   return n;
} /* end rs41date() */
...
static void decode41(uint32_t m)
{
   ...
   { /* with */
      ...
      do {
         ...
         encr = 0;
         aux = 0UL;
         satc = X2C_max_longcard;
         newvers = 0;
         if (try0>0UL) {
            if (try0>1UL) {
               for (i = 0UL; i<=559UL; i++) {
                  if ((uint32_t)anonym->fixcnt[i]>=10UL) {
         ...
         for (;;) {
            ...
            */
            ...
            if (verb2) {
               osi_WrStrLn("", 1ul);
               osi_WrStrLn("", 1ul);
               osi_WrStr("crc ok, start ID length data... crc [", 38ul);
            ...
            if (i<=tmp) for (;; i++) {
               ...
               if (anonym->fixbytes[i]==anonym->rxbuf[i]) {
                  if ((uint32_t)anonym->fixcnt[i]<255UL) {
                     ++anonym->fixcnt[i];
                  }
               }
               else {
            ...
            if (typ=='y') {
               ...
               */
               ...
               /*             WrStrLn("7D gps raw"); */
               posok = p;
               satc = (uint32_t)(uint8_t)anonym->rxbuf[p+18UL];
            }
            else if (typ=='\202') {
               posok = p;
               date = rs41date(anonym->rxbuf, 560ul, p+18UL);
            }
            else if (typ=='\203') {
               newvers = 1;
               satc = 0UL;
               for (i = 0UL; i<=31UL; i++) {
                  /* gps prn number range */
                  if (X2C_IN(i&7UL,8,
                (uint8_t)(uint8_t)anonym->rxbuf[p+18UL+i/8UL])) ++satc;
               } /* end for */
            }
            else if (typ=='~') ++aux;
            else if (typ=='x') {
      ...
      if (verb && nameok>0UL) {
         ...
         if (satc<100UL) {
            osic_WrUINT32(satc, 3UL);
            if (newvers) osi_WrStr("Sat(s) ", 8ul);
            else osi_WrStr("Sats ", 6ul);
         }
         ...
         osi_WrStr(" toa:", 6ul);
         osic_WrUINT32(anonym->toaus&16777215UL, 1UL);
         osi_WrStr("/", 2ul);
         osic_WrUINT32(anonym->toaus/16777216UL, 1UL);
         WrdB(chan[m].admax-chan[m].admin);
...
#define sondeudp_CHECKLEN 318
/* min rs41 frame len for quality sample and toa */
         ...
         if (anonym->rxbitc>=8UL) {
            ...
            if (anonym->rxp==318UL) {
               anonym->bitlev0 = anonym->bitlev;
                /* save quality before end of shortest frame */
               anonym->noise0 = anonym->noise;
               toa = X2C_DIVL((double)sampcount,
                (double)adcrate)-X2C_DIVL(((double)
                chan[m].r92.baudfine-(X2C_DIVL((double)
                anonym->pllupdown,
                (double)anonym->updowncnt))*1024.0)*7.62939453125E-6,
                (double)anonym->configbaud);
               /*          IF toa<0.0 THEN toa:=toa+1.0 ELSIF toa>1.0 THEN toa:=toa-1.0 END;
                 */
               /*          toaus:=ASH(sampcounth, 24) + VAL(CARDINAL,
                10000000.0*toa);   (* 24 lsb toa + 8msb seconds from samples *) */
               toa = (toa+(double)(sampcounth&255UL))*1.6777216E+7;
               if (toa<0.0) toa = toa+4.294967295E+9;
               else if (toa>4.294967295E+9) toa = toa-4.294967295E+9;
               anonym->toaus = (uint32_t)X2C_TRUNCC(toa,0UL,
                X2C_max_longcard);
            }
            anonym->rxbitc = 0UL;
         }
   ...
   */
   { /* with */
      ...
      if (anonym->cbit) {
         ...
         ++chan[m].r41.updowncnt;
         if (d!=anonym->oldd) {
            if (d==anonym->plld) {
               anonym->baudfine += anonym->pllshift;
               chan[m].r41.pllupdown += (int32_t)chan[m].r41.updowncnt;
            }
            else {
               anonym->baudfine -= anonym->pllshift;
               chan[m].r41.pllupdown -= (int32_t)chan[m].r41.updowncnt;
...
static void Fsk(uint32_t m)
{
   ...
   { /* with */
      ...
      while (anonym->baudfine>=65536L) {
         anonym->baudfine -= 65536L;
         ff = Fir(afin, (uint32_t)anonym->baudfine/1024UL, 64UL,
                chan[m].afir, 32ul, anonym->afirtab, 2048ul);
         demod9241(ff, m);
      }
   }
} /* end Fsk() */
...
static void checkdf69(float alti, uint32_t m)
{
   chan[m].nonames->dfm6.d9 = alti<1.0f;
                /* if alti<1 we have swapped values */
} /* end checkdf69() */
                ...
                uint32_t subnum)
{
   ...
   case 4UL:
      if (chan[m].nonames->dfm6.d9) {
         ...
         /* dfm06 alt, clb */
         v = bits2val(b, b_len, 0UL, 32UL);
         checkdf69((float)v*0.01f, m); /* test if dfm6 or dfm9 */
         ui = (int32_t)bits2val(b, b_len, 32UL, 16UL);
         if (ui>=32768L) ui -= 65536L;
         if (verb) {
            osi_WrStr(" alti:", 7ul);
      ...
      */
      ...
      if (i<anonym->nameregtop) {
         { /* with */
            ...
            if (ix<=1UL && ((uint32_t)
                anonym0->cnt[ix]==0UL || anonym0->dat[ix]==d)) {
               anonym0->dat[ix] = d;
               if ((uint32_t)anonym0->cnt[ix]<255UL) ++anonym0->cnt[ix];
               if (verb2 || verb) {
                  /* & (idcnt0=0)*/
                  osi_WrStr(" ID:", 5ul);
                  osi_WrStr((char *)(tmp = hex((uint32_t)st/16UL),
                &tmp), 1u/1u);
   ...
   { /* with */
      ...
      for (;;) {
         if (anonym->baudfine>=65536L) {
            anonym->baudfine -= 65536L;
            ff = Fir(afin, (uint32_t)((anonym->baudfine&65535L)/1024L),
                64UL, chan[m].afir, 32ul, anonym->afirtab, 2048ul);
            demod6(ff, m);
         }
         anonym->baudfine += lim;
         lim = 0L;
   ...
   { /* with */
      ...
      if (ok0) {
         ...
         if ((anonym->c50 && anonym->id50.id[0U])
            ...
            flen = 22UL;
            sdrparm(s, 101ul, &flen, (int32_t)channel, 0);
...
static void Afsk(uint32_t channel)
{
   ...
   { /* with */
      struct C34A * anonym = &chan[channel].c34a;
      right = Fir(afin, 0UL, 64UL, chan[channel].afir, 32ul, anonym->afirtab,
                 2048ul);
      if (anonym->left<0.0f!=right<0.0f) {
         d = X2C_DIVR(anonym->left,anonym->left-right);
         a = (float)(uint32_t)X2C_TRUNCC(d*64.0f+0.5f,0UL,
                X2C_max_longcard);
         b = a*1.5625E-2f;
         if ((uint32_t)X2C_TRUNCC(a,0UL,
                X2C_max_longcard)>0UL && (uint32_t)X2C_TRUNCC(a,0UL,
                X2C_max_longcard)<64UL) {
            mid = Fir(afin, 64UL-(uint32_t)X2C_TRUNCC(a,0UL,
                X2C_max_longcard), 64UL, chan[channel].afir, 32ul,
                anonym->afirtab, 2048ul);
...
static void decodeframe10(uint32_t m)
{
   ...
   { /* with */
      ...
      for (;;) {
         ...
         if ((anonym->alternativ || crcok) || (uint32_t)repairstep==0UL) {
            break;
         }
         repl = 0UL;
         for (i = 0UL; i<=98UL; i++) {
            if (!X2C_INL(i,256,_cnst2) && (uint32_t)anonym->fixcnt[i]>=(uint32_t)repairstep) {
               /* replace stable bytes */
         ...
         repairstep = (uint8_t)((uint32_t)repairstep/2UL);
                /* make next crc check */
      }
      if (crcok) {
         /*INC(test1); WrInt(test1, 6); WrStrLn(" testcnt");  */
         ...
         for (i = 0UL; i<=98UL; i++) {
            if (anonym->fixbytes[i]==anonym->rxbuf[i]) {
               if ((uint32_t)anonym->fixcnt[i]<255UL) ++anonym->fixcnt[i];
            }
            else {
               anonym->fixbytes[i] = anonym->rxbuf[i];
               anonym->fixcnt[i] = 0U;
         ...
         */
         ...
         if (anonym->txok) {
            ...
            flen = 117UL;
            sdrparm(s, 201ul, &flen, (int32_t)m, 0);
      ...
      if (frl>2UL) {
         for (;;) {
            ...
            crcok = (uint32_t)crcm10((int32_t)(frl-2UL), anonym->rxbuf,
                ...
                2L);
            if (crcok || (uint32_t)repairstep==0UL) {
               /*alternativ OR*/
               break;
            }
            repl = 0UL;
            ...
            do {
               if (!X2C_INL(i,256,
                _cnst6) && (uint32_t)anonym->fixcnt[i]>=(uint32_t)
                repairstep) {
                  /* replace stable bytes */
                  if (anonym->rxbuf[i]!=anonym->fixbytes[i]) {
                     ++repl;
...
static void meisname(uint32_t n, char s[], uint32_t s_len)
{
   ...
   if (sn>=1.0f && sn<=1.6777215E+7f) {
      n = (uint32_t)X2C_TRUNCC(sn+0.5f,0UL,X2C_max_longcard)&16777215UL;
...
static void decodemeisei(uint32_t m)
{
   ...
   { /* with */
      ...
      j = 0UL;
      for (bk = 0UL; bk<=5UL; bk++) {
         if ((int32_t)blockok[bk]>=2L) ++j;
         else if ((int32_t)blockok[bk]>=0L) {
            j += 2UL;
            okbits |= (1U<<bk);
         }
      } /* end for */
      ...
      if (verb1) {
         ...
         osi_WrStr(" ", 2ul);
         for (i = 0UL; i<=5UL; i++) {
            if ((int32_t)blockok[i]==0L) osi_WrStr("+", 2ul);
            else if ((int32_t)blockok[i]==1L) osi_WrStr("1", 2ul);
            else if ((int32_t)blockok[i]==2L) osi_WrStr("2", 2ul);
            else if ((int32_t)blockok[i]==-1L) osi_WrStr("-", 2ul);
            else osi_WrStr("!", 2ul);
         } /* end for */
         if (verb2) {
            osi_WrStrLn("", 1ul);
      ...
      if (anonym->frametyp==1UL) {
            ...
            if (anonym->frametyp==1UL) {
               ...
               if (verb1) {
                ...
                (j%1000000UL)*1.6666666666667E-6;
                  ...
                  if ((int32_t)blockok[1U]>=0L && (int32_t)
                blockok[2U]>=0L) {
                     j = 0UL;
                     for (i = 0UL; i<=3UL; i++) {
                ...
                (j%1000000UL)*1.6666666666667E-6;
                  ...
                  if ((int32_t)blockok[2U]>=0L && (int32_t)
                blockok[3U]>=0L) {
                     j = 0UL;
                     if (anonym->subtype==1UL) {
                        for (i = 0UL; i<=2UL; i++) {
                  ...
                  if ((anonym->subtype==1UL && (int32_t)blockok[3U]>=0L)
                && (int32_t)blockok[4U]>=0L) {
                     if (verb2) {
                        osi_WrStr(" [", 3ul);
                        for (i = 13UL; i<=17UL; i++) {
                           osi_WrHex((uint32_t)bb[i], 1UL);
                ...
                bb[20U]*256UL+(uint32_t)bb[21U])*1.851984E-2;
                  ...
                  else if ((int32_t)blockok[4U]>=0L) {
                     dg = (double)((uint32_t)
                bb[14U]*256UL+(uint32_t)bb[15U])*0.01;
                     osi_WrStr(" kmh:", 6ul);
                     osic_WrFixed((float)dg, 2L, 1UL);
                  }
                  if (anonym->subtype!=1UL && (int32_t)blockok[4U]>=0L) {
         ...
         if ((int32_t)blockok[0U]>=0L) {
            ...
            if ((int32_t)blockok[3U]>=0L && (uint32_t)
                bb[14U]==48UL+(uint32_t)(anonym->frametyp==2UL)) {
               anonym->subtype = 1UL+(uint32_t)((uint32_t)bb[15U]<=192UL)
                ;
               if (verb1) {
            ...
            if (anonym->subtype==1UL && (int32_t)blockok[1U]>=0L) {
            ...
            if ((int32_t)blockok[5U]>=0L && anonym->frametyp==0UL) {
