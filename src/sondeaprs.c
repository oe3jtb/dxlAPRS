...
static void wrcsv(uint32_t sattime, const char typstr[],
                ...
                uint32_t ogstr_len, const char toadata[],
                uint32_t toadata_len)
{
   int32_t fd;
   uint32_t j;
   uint32_t i;
   ...
   if (sondeaprs_json) {
      ...
      if (toadata[0UL]) {
         aprsstr_Append(s, 1000ul, ",\"toa\":\"", 9ul);
         aprsstr_Append(s, 1000ul, toadata, toadata_len);
         aprsstr_Append(s, 1000ul, "\"", 2ul);
      }
      ...
      if (f>=0L) {
         ...
         while (eol+2L>=bol && fb[bol]=='%') {
            if (fb[bol+1L]=='u') {
               ...
               */
               /* insert version */
               aprsstr_Append(hb, 160ul, " sondemod 1.37m", 16ul);
            }
            else if (fb[bol+1L]=='s') {
               /* insert sat count */
               if (sats>0UL) {
...
static void sendaprs(uint32_t comp0, uint32_t micessid, char dao,
                ...
                uint32_t * commentcnt, const struct sondeaprs_SDRBLOCK sdr,
                 double myazi, double myele, double mydist,
                 int32_t txtime, const char toadata[],
                uint32_t toadata_len)
{
   char ds[255];
   char h[255];
   ...
   if (toadata[0UL]==0) {
      ...
      aprsstr_Append(b, 255ul, " toa=", 6ul);
      aprsstr_Append(b, 255ul, toadata, toadata_len);
   }
   /* limit to fit in pr */
...
extern void sondeaprs_senddata(double lat, double long0,
                ...
                struct sondeaprs_XDATA xdata, char toadata[],
                uint32_t toadata_len)
{
   uint8_t e;
   uint32_t nt;
   ...
   if (!egmoff) {
      ...
      if (altNN>(-1000.0)) {
         ...
         if (og>=0.0) btalt = og;
         if ((uint32_t)attr==3UL) strncpy(ogwhat,"Water",251u);
         else if ((uint32_t)attr==2UL) strncpy(ogwhat,"Wood",251u);
         else if ((uint32_t)attr==1UL) strncpy(ogwhat,"Urban",251u);
      }
   ...
   if (ct) {
      { /* with */
         ...
         if (toadata[0UL] && bt>10UL) bt = 10UL;
         if ((bt>0UL && anonym->lastbeacon+bt<=systime)
            ...
            /*
                    ...
                    END;
            */
            if ((calperc>0UL && calperc<100UL) && toadata[0UL]==0) {
            ...
            apptxon(txtime, anonym->cyclevals, s, 251ul);
                /* remaining tx time */
            if ((vBatt>0.0 && anonym->cyclevals%3UL==0UL) && toadata[0UL]==0)
                 {
