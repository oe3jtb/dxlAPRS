...
static void aprs(const struct FLY f)
{
   ...
   altegm = f.alt;
   /*
     IF srtmdir[0]<>0C THEN altegm:=egmcorr(pos, FALSE, f.alt) END;
   */
   if (aprspos_posvalid(homepos)) {
...
static void store(const CSV csv0)
{
   ...
   if ((((csv0[0U][0U]=='M' && csv0[0U][1U]=='S') && csv0[0U][2U]=='G')
      ...
      /*IF verb2 & (csv[17][0]<>0C) THEN WrStr("Squawk:"); WrStrLn(csv[17]);
...
extern int main(int argc, char **argv)
{
   ...
   if ((aprspos_posvalid(homepos) && libsrtm_srtmdir[0U])
      ...
      if (homealt<=(-1000.0f) || homealt>=10000.0f) {
         osi_WrStrLn("no SRTM alitude for home found", 31ul);
      }
      homealtwgs = egmcorr(homepos, 1, homealt); /* altitude to wgs84 */
   }
   if ((verb && homealt>(-1000.0f)) && homealt<10000.0f) {
