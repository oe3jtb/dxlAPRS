...
able) pipe", 103ul);
 ...
 (automatic)", 105ul);
               ...
               osi_WrStrLn(" -T                 send json udp with DCD-state+sf valid for this sf", 70ul);
...
1", 94ul);
   ...
   while (pd0) {
      ...
      if (pd0->frames[0U].cfgsf<=8UL) {
         pd0->frames[0U].afcspeed = pd0->frames[0U].afcspeed*0.5f;
      }
...
static void senddcd(int32_t sf, char inv, char on)
{
   char h[100];
   char s[100];
   int32_t ret;
   if (judpport) {
      ...
      ret = udpsend(udpsock, s, (int32_t)aprsstr_Length(s, 100ul), judpport, jipnum);
   }
} /* end senddcd() */
                ...
                uint32_t from, uint32_t too, char revers)
{
   ...
   while (i0<=too) {
      j1 = i0;
      if (revers) j1 = (too-i0)+from;
      if (j1<=text_len-1) WrHex((uint32_t)(uint8_t)text[j1], 2UL, 0UL);
      ++i0;
   }
} /* end WH() */
...
{
   uint32_t i0;
   if (textlen>=15UL && verb) {
      WH0("Meshtastic: Dest:", 18ul, text, text_len, 0UL, 3UL, 1);
      WH0(" SRC:", 6ul, text, text_len, 4UL, 7UL, 1);
      WH0(" MID:", 6ul, text, text_len, 8UL, 11UL, 1);
      i0 = (uint32_t)(uint8_t)text[12UL];
      osi_WrStr(" HopLim:", 9ul);
      osic_WrINT32(i0&7UL, 1UL);
      osi_WrStr(" HopStart:", 11ul);
      osic_WrINT32(i0/32UL, 1UL);
      if ((i0/8UL&1)) osi_WrStr(" WantAck", 9ul);
      if ((i0/16UL&1)) osi_WrStr(" viaMQTT", 9ul);
...
static void encodeaprs(int32_t qual, int32_t afc, float level, int32_t txd, const struct FFRAME frame,
                ...
                float temp, float hum, float baro, int32_t fecc, char dcd)
{
   char h[501];
   char b[501];
   uint32_t n;
...
static void frameout(struct FFRAME * frame, const char finf[], uint32_t finf_len, char hascrc,
                char crc0, char dcdlost, char opt, char dcd, uint32_t cr, char text[],
                 uint32_t text_len)
{
   uint32_t td;
   uint32_t blocksize;
   ...
   if (frame->dlen<text_len-1) {
      ...
      if (verb && (!quietcrc || hascrc && crc0)) {
         ...
         if (frame->illid==0UL) {
            ...
            WrHexCap(frame->illid*256UL+frame->idfound, 4UL, 0UL);
         }
         osi_WrStr(" cr:", 5ul);
         osic_WrINT32(cr, 1UL);
         osi_WrStr(" len:", 6ul);
...
static char decodechirp(struct FFRAME * frame, const struct BINS bins, char opti, char alldcd)
{
   int32_t o;
   uint32_t burst;
   uint32_t crctry;
      ...
      if (anonym->chirpc==0UL) {
         ...
         if (!dcd || anonym->nibbc>=fulllen*2UL) {
            ...
            if (anonym->nibbc>1UL) {
               ...
               frameout(frame, anonym->fecinfo, 96ul, anonym->withcrc, crcok, !dcd, opti, alldcd, cr, text, 261ul);
            }
            return 1;
         }
      }
   ...
   if (isize==1UL) {
      if (u8signed) {
         ...
         if (i0<=tmp) for (tmp = (uint32_t)(tmp-i0)/2UL;;) {
            ...
            b[*wp].Im = (float)(int32_t)(signed char)ib.b[i0+1UL];
...
static char nextchirp(struct FFRAME * frame, char alldcd)
{
   struct BINS bins;
   uint32_t blocksize;
   uint32_t i0;
   ...
   if (frame->state==lorarx_sHUNT) {
      ...
      if (decodechirp(frame, bins, opt, alldcd)) frame->state = lorarx_sSLEEP;
      ++frame->dcnt;
   }
   ++frame->cnt;
   return 0;
            ...
            while (pd) {
               ...
               if (i<=pd->coldet && ((i==0UL || pd->frames[i].state>=lorarx_sSYNRAW)
                ...
                pd->frames[j].fp)<=1L) {
                  ...
                  allcol = 0;
                  pd->done = pd->done || nextchirp(&pd->frames[i], pd->dcdstate);
      ...
      if (sendjudpdcd) {
         ...
         while (pd) {
            { /* with */
               ...
               if (anonym->dcdstate!=allcol) {
                  senddcd((int32_t)anonym->frames[0U].cfgsf, anonym->frames[0U].invertiq, allcol);
               }
