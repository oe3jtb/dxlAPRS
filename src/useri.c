...
{
   if (configs[v].typ==useri_cBOOL) return (uint32_t)configs[v].on>0UL;
   else if (configs[v].typ==useri_cBLINE && configs[v].lines) {
      return (uint32_t)configs[v].lines->active>0UL;
   }
   else return 0;
   return 0;
} /* end configon() */
...
static char configson(uint8_t v, uint32_t line)
{
   ...
   for (;;) {
      if (pl==0) break;
      if (line==0UL) return (uint32_t)pl->active>0UL;
      --line;
      pl = pl->next;
   }
   return 0;
...
{
   if (pl) pl->active = (uint32_t)pl->active==0UL;
} /* end linenoton() */
      ...
      if (configs[i].title[0U] && i!=useri_fEDITLINE) {
         ...
         for (;;) {
            ...
            if (configs[i].typ==useri_cLIST || configs[i].typ==useri_cBLIST) {
               if (pl) c = (char)(((uint32_t)pl->active&7UL)+48UL);
            }
            else c = (char)(((uint32_t)configs[i].on&7UL)+48UL);
   ...
   for (;;) {
      ...
      if (c=='|') inh = 1U;
         ...
         if (c==':') {
            ...
            else if ((uint32_t)inh==2UL) {
               c = '\376';
               inh = 0U;
            }
         }
      }
      if ((uint32_t)inh!=1UL && (c=='\012' || (uint8_t)c>=' ')) {
         /* & ((tp<>0) OR (c<>LF))*/
         s[tp] = c;
         ++tp;
         if (tp>=2000UL) break;
...
static void infosdo(pMENU menu)
{
   ...
   if ((0x100U & what)) {
      if ((uint32_t)lastval.dust10>0UL) {
      ...
      if ((uint32_t)lastval.dust01>0UL) {
         ...
         if (x<=tmp0) for (;; x++) {
            { /* with */
               ...
               if (allcolor || !(anonym->g&1)) {
                  if ((uint32_t)anonym->r>1024UL) anonym->r = 1024U;
                  if ((uint32_t)anonym->g>1024UL) anonym->g = 1024U;
                  if ((uint32_t)anonym->b>1024UL) anonym->b = 1024U;
                  anonym->r = (uint16_t)(1024UL-(uint32_t)anonym->r);
                  anonym->g = (uint16_t)(1024UL-(uint32_t)anonym->g);
                  anonym->b = (uint16_t)(1024UL-(uint32_t)anonym->b);
               }
            }
            if (x==tmp0) break;
         } /* end for */
                ...
                uint32_t subknob, char folded)
{
   ...
   if (knob==1UL) {
      if (subknob==0UL) {
         ...
         if (useri_beaconed) {
            aprstext_encbeacon(s, 1000ul, &len);
            if (s[0U]) aprsdecode_drawbeacon(s, 1000ul, 0);
         }
         else {
...
/* do beacon ed fold menu */
{
   ...
   else if (knob==3UL) aprsdecode_drawbeacon(s, 301ul, 0);
                ...
                uint32_t * ypos)
{
   ...
   *subknob = 0UL;
   if (*m && (*knob>0UL || (*m)->fullclamp)) {
      while ((*subknob<=20UL && (uint32_t)(*m)
                ->subk[*knob-1UL][*subknob]>0UL) && *xpos>(uint32_t)(*m)
                ->subk[*knob-1UL][*subknob]) ++*subknob;
      focuswid = (*m)->wid;
   }
   else focuswid = 0UL;
...
static void configeditor(void)
{
   ...
   while (cnt>0UL) {
      ...
      if (configedit==137UL) {
         ...
         if (((uint32_t)pl->active/2UL&1)) {
            aprsstr_Append(s, 201ul, " +", 3ul);
         }
         else aprsstr_Append(s, 201ul, "  ", 3ul);
         aprsstr_Append(s, 201ul, "Focus |", 8ul);
         if (((uint32_t)pl->active/4UL&1)) {
            aprsstr_Append(s, 201ul, " +", 3ul);
         }
         else aprsstr_Append(s, 201ul, "  ", 3ul);
      ...
      if (onof) onoff(m, 8L, (uint32_t)pl->active>0UL);
      pl = pl->next;
      --cnt;
   }
   m->hiknob = hks;
...
extern void useri_findopl(uint32_t where)
{
   ...
   newmenu(&menu, aprsdecode_lums.fontxsize*67UL+4UL,
...
/* has any watchcall "follow" enabled" */
{
   ...
   for (;;) {
      ...
      if (wc[0U]==0) break;
      if (((uint32_t)useri_confflags(useri_fWATCH, j)/2UL&1)) return 1;
...
static void statusbar(void)
{
   ...
   for (i = useri_fUDP1; i<=useri_fUDP4; i++) {
      /*    statuscoly(menu^.image, xknob*(kx+1), xknob,
                realtime-udpsocks[kx].lastudptx, */
      /*               realtime-udpsocks[kx].lastudprx, configon(i)); */
      if (useri_configon(i) && (int32_t)aprsdecode_udpsocks0[(uint32_t)
                i-39UL].fd<0L) {
         /* port open but got no socket */
         c.r = 300U;
         c.g = 300U;
         c.b = 0U;
         statuscol(menu->image, (int32_t)(xknob*(kx+1UL)),
                (int32_t)xknob, c);
      }
      else {
         statuscoly(menu->image, (int32_t)(xknob*(kx+1UL)),
                (int32_t)xknob,
...
static void mouseshow(uint32_t x, uint32_t y)
{
   ...
   if (aprspos_posvalid(pos)) {
      ...
      useri_textautosize(-3L, 0L, 19UL, 30UL, 'g', s, 151ul);
      s[0U] = 0;
      pos1 = pos;
      ...
      if (idx<=158UL) {
         { /* with */
            ...
            { /* with */
               ...
               le = 200UL;
               if ((uint32_t)anonym->width>0UL && 200UL>(uint32_t)
                anonym->width) le = (uint32_t)anonym->width;
               if (le==0UL) le = 1UL;
               if ((uint32_t)anonym->curspos>=le) {
                  anonym->curspos = (uint16_t)(le-1UL);
               }
               if (*ch=='\023' || *ch=='\311') {
                  if ((uint32_t)anonym->curspos>0UL) --anonym->curspos;
               }
                  ...
                  if ((uint8_t)anonym0->line[anonym->curspos]<=' ') {
                     while ((uint32_t)anonym->curspos>0UL && (uint8_t)
                anonym0->line[(uint32_t)anonym->curspos-1UL]<=' ') {
                        --anonym->curspos;
                     }
                  }
               }
               else if (*ch=='\010') {
                  if ((uint32_t)anonym->curspos>0UL) {
                     /*            knoboff(idx, pm); */
                     tmp = le-1UL;
               ...
               if ((uint8_t)*ch>=' ' && (uint8_t)*ch<'\177') {
                  /*          knoboff(idx, pm); */
                  if (!overtype) {
                     tmp = (uint32_t)anonym->curspos+1UL;
                     i = le-1UL;
                     if (i>=tmp) for (;; i--) {
         ...
         if (y+yof<xbufysize) {
            ...
            while (x<xl) {
               { /* with */
                  ...
                  if ((uint32_t)anonym->b!=65535UL) {
                     rh0 = anonym->r;
                     gh = anonym->g;
                     bh = anonym->b;
                     if (transp && (uint32_t)bh>0UL) {
                        { /* with */
                ...
                *mainimg->Len0+((mainimg->Len0-1)-(y+yof))];
                 ...
                 bm = (uint16_t)aprsdecode_lums.menubackcol.b;
                           rh0 += (uint16_t)((uint32_t)rm/2UL);
                           gh += (uint16_t)((uint32_t)gm/2UL);
                           bh += (uint16_t)((uint32_t)bm/2UL);
                        }
                     }
                     if ((uint32_t)bh>=1024UL) bh = 1023U;
                     if ((uint32_t)rh0>=1024UL) rh0 = 1023U;
                     if ((uint32_t)gh>=1024UL) gh = 1023U;
                     /*
   ...
   if (menu==0) {
      ...
      if (menu->wid==225UL) dolist(menu, potx, poty);
         ...
         */
         ...
         if (subknob==0UL) {
            configs[useri_fSRTMCACHE].on = (uint8_t)(((uint32_t)
                configs[useri_fSRTMCACHE].on+1UL)%3UL);
            updatemenus();
            useri_rdlums();
         }
         else if (subknob==1UL) {
