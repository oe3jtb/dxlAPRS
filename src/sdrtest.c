...
extern int main(int argc, char **argv)
{
   ...
   freqc = 0UL;
   errc = sdr_startsdr(url, 1001ul, port, 1001ul, iqrate, samphz, reconn,
                f32);
   if (errc>=0L) {
      /*    rdconfig; */
      tshow = 0UL;
      ...
      if (soundfd>=0L || udpsounds) {
         ...
         for (;;) {
            ...
            if (sn<0L) {
               if (verb) {
                  if (sn==-2L) osi_WerrLn("end of input file", 18ul);
      ...
      osi_WerrLn("not implemented input samplerate, try (slow) float input",
                57ul);
   }
