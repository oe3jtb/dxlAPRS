   ...
   for (;;) {
      ...
      if (h[0U]=='-' && h[1U]) {
         if (h[1U]=='p') {
            ...
            osi_NextArg(h, 4096ul);
            if ((!aprsstr_StrToCard(h, 4096ul, &chip.lnagain) || chip.lnagain>6UL) || chip.lnagain<1UL) {
            ...
            osi_NextArg(h, 4096ul);
            if (!aprsstr_StrToCard(h, 4096ul, &roll)) {
               Error("-r <lines>", 11ul);
            }
         }
         else if (h[1U]=='s') {
...
extern int main(int argc, char **argv)
{
   ...
   for (;;) {
      if (mescnt>1000UL) {
         /* chip destroys some registers so reload is */
         mescnt = 0UL;
         scpo(chip.gpio, 26UL, 18UL);
         scpo(chip.gpio, 12UL, 32UL*(chip.lnagain&7UL)+(uint32_t)chip.lnaboost*3UL);
                /* modifies self... lna gain, current */
