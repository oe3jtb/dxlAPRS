   ...
   for (;;) {
      ...
      if ((h[0U]=='-' && h[1U]) && h[2U]==0) {
         if (h[1U]=='m' || h[1U]=='r') {
            ...
            if (h[1U]=='h') {
               osic_WrLn();
               osi_WrStrLn("Read serial GPS and make normal/compressed/mic-e/meshcom4 Beacon in AXUDP or text", 82ul);
                ...
                uint32_t comm_len, const char viapath[], uint32_t viapath_len, char local, pUDPDESTS udp0, char symt0, char symb0, uint32_t meshcom4)
{
   ...
   if (aprsstr_Length(mycall, 100ul)>=3UL) {
      while (udp0) {
         ...
         if (rp0>2L) sendudp(raw, 361ul, rp0, udp0->ipnum, udp0->toport);
         else osi_WrStrLn("axudp encode error (maybe wrong callsign or symbol)", 52ul);
         udp0 = udp0->next;
      }
   }
   if (verb) {
...
static void initubloxraw(const uint8_t cmd[], uint32_t cmd_len)
{
   ...
   sum1 = (uint8_t)((uint32_t)b[2U]+(uint32_t)b[3U]);
   sum2 = (uint8_t)((uint32_t)b[2U]*2UL+(uint32_t)b[3U]);
   i = 4UL;
   while (i<=j+3UL) {
      sum1 += b[i];
      sum2 += sum1;
         ...
         if (gpsp>(uint32_t)(uint8_t)gpsb[4U]+(uint32_t)(uint8_t)gpsb[5U]*256UL+7UL) {
            ...
            if ((uint32_t)sum1==(uint32_t)(uint8_t)gpsb[gpsp-2UL] && (uint32_t)sum2==(uint32_t)(uint8_t)gpsb[gpsp-1UL]) {
               ...
               if (((uint32_t)(uint8_t)gpsb[2U]==1UL && (uint32_t)(uint8_t)gpsb[3U]==6UL) && gpsp==60UL) {
                  ...
                  pos.dateok = pos.fix>0UL && (uint32_t)pos.timeflags==7UL;
