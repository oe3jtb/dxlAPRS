...
#define sondemod_MYLAT 8.3775804095728E-1
/* only for show rs92 sat elevations if no pos decode */

#define sondemod_MYLONG 2.2689280275926E-1
...
r>", 55ul);
            ...
            osi_NextArg(testencrname, 9ul);
            if ((uint8_t)testencrname[0U]<' ') {
               Error("-U <toa-test-rs41-name>", 24ul);
            }
         }
         else if (h[1U]=='P') {
            osi_NextArg(h, 1024ul);
...
SENDING TO THE WORLD!", 71ul);
               osi_WrStrLn(" -G <km>        send only if Ground-distance to S\
onde not more, 0=off (needs -P) (-G 15)", 89ul);
...
static void initcontext(struct CONTEXTR9 * cont)
{
   ...
   cont->lastlat = 8.3775804095728E-1;
   cont->laslong = 2.2689280275926E-1;
} /* end initcontext() */
...
{
   if ((p1!=0.0f && p2!=0.0f) && (float)fabs(p1-p2)>1.5707963267949E-3f) {
      *cnt += *cnt+1UL;
      if (*cnt>60UL) *cnt = 60UL;
   }
   else if (*cnt>0UL) --*cnt;
} /* end jumpcheck() */


static void checkdf69(float alti, char * df9)
{
   *df9 = alti==0.0f; /* if alti<1 we have swapped values */
} /* end checkdf69() */
   ...
   { /* with */
      ...
      if (i<anonym->nameregtop) {
         { /* with */
            ...
            else if (((uint32_t)anonym0->errcnt==0UL && (uint32_t)
                anonym0->cnt[0U]>10UL) && (uint32_t)anonym0->cnt[1U]>10UL) {
      ...
      while (i<anonym->nameregtop) {
         { /* with */
            ...
            min0 = anonym2->cnt[0U];
            if ((uint32_t)min0>(uint32_t)anonym2->cnt[1U]) {
               min0 = anonym2->cnt[1U];
            }
            if ((uint32_t)min0>(uint32_t)max0) {
               max0 = min0;
               v = i;
            }
         }
      ...
      if ((uint32_t)max0>2UL && (uint32_t)anonym->namereg[v].errcnt==0UL)
                 {
         /* may be good enough */
         aprsstr_Assign(ser, ser_len, "[  ]", 5ul);
         ser[1UL] = hex((uint32_t)anonym->namereg[v].start/16UL);
         ...
         for (;;) {
            ...
            if ((uint32_t)anonym->namereg[v].start/16UL==dftyp->n) {
...
/*PROCEDURE fulltoa(t:CARDINAL):CARDINAL; */
/*BEGIN RETURN t MOD 1000000H + ASH(t,-24) * 10000000 END fulltoa; */

static void enc64(uint32_t n, uint32_t d, char s[],
                uint32_t s_len)
{
   uint32_t x;
   char tmp;
   while (d>0UL) {
      ...
      n = n/64UL;
      if (x<10UL) {
         aprsstr_Append(s, s_len, (char *)(tmp = (char)(x+48UL),
                &tmp), 1u/1u);
      }
      else if (x<36UL) {
         aprsstr_Append(s, s_len, (char *)(tmp = (char)(x+55UL),
                &tmp), 1u/1u);
      }
      else if (x<62UL) {
         aprsstr_Append(s, s_len, (char *)(tmp = (char)(x+61UL),
                &tmp), 1u/1u);
      }
      else if (x<63UL) aprsstr_Append(s, s_len, "+", 2ul);
...
static void findencrypted(pCONTEXTR4 pc, char s[], uint32_t s_len)
/* vers[1]name[0..9],freq10khz(3)lat[4]long[4]alt[2],fn[3]dt[4].... */
{
   pCONTEXTR4 pe;
   uint32_t j;
   uint32_t i;
   ...
   struct _0 * anonym;
   s[0UL] = 0;
   memset((char *)fns,(char)0,sizeof(struct _0 [11]));
   if (pc->encrcnt==0UL) {
      pe = pcontextr4;
      while (pe) {
         if (pe->encrcnt>0UL) {
            /* rs41 and rs41 encr found */
            for (i = 0UL; i<=9UL; i++) {
               for (j = 0UL; j<=9UL; j++) {
                  if (((pc->toas[i].udpip==pe->toas[j].udpip && pc->toas[i]
                .udpport==pe->toas[j].udpport) && labs((int32_t)
                (pc->toas[i].date-pe->toas[j].date))<2L)
                && pe->framenum-pe->toas[j].framenum<10UL) {
                     /* from same sondeudp */
                     /* from same time intervall */
                     /* max frames */
                     ii = (int32_t)(pc->toas[i].toa-pe->toas[j].toa);
                     if (labs(ii)<5000000L) {
                        { /* with */
                           struct _0 * anonym = &fns[pe->framenum-pe->toas[j]
                .framenum];
                           anonym->dt = ii;
                           anonym->fnt = pe->toas[j].framenum;
                        }
                     }
                  }
               } /* end for */
            } /* end for */
            /*WrStr("[");WrInt(pe^.framenum-pe^.toas[j].framenum,1);
                WrStr("/");WrInt(dt,1);WrStr("]"); */
            /*WrStr("[");WrInt(pe^.toas[j].framenum,1);WrStr("/");
                WrInt(dt,1);WrStr("]"); */
            /*WrStrLn("***"); */
            i = 0UL;
            j = 0UL;
            ...
            if (j>0UL) {
               while (fns[j].fnt==0UL) --j;
               aprsstr_Append(s, s_len, "1", 2ul); /* frame format version */
               aprsstr_Append(s, s_len, pe->name, 9ul); /* xsonde name */
               aprsstr_Append(s, s_len, ",", 2ul);
               enc64((uint32_t)X2C_TRUNCC(pe->mhz0*100.0f+0.5f,0UL,
                X2C_max_longcard), 3UL, s, s_len);
               enc64((uint32_t)
                X2C_TRUNCC((sondeaprs_mypos.lat+1.5707963267949f)
                *5.1566201561774E+6f,0UL,X2C_max_longcard), 4UL, s, s_len);
               enc64((uint32_t)
                X2C_TRUNCC((sondeaprs_mypos.long0+3.1415926535898f)
                *2.5783100780887E+6f,0UL,X2C_max_longcard), 4UL, s, s_len);
               enc64((uint32_t)X2C_TRUNCC(sondeaprs_myalt*0.25f,0UL,
                X2C_max_longcard), 2UL, s, s_len);
               /*          Append(s, ","); enc64(pe^.framenum, 3, s); */
               /*i:=0; */
               aprsstr_Append(s, s_len, ",", 2ul);
               enc64(fns[i].fnt, 3UL, s, s_len);
               do {
                  if (fns[i].fnt>0UL) {
                     enc64((uint32_t)fns[i].dt&16777215UL, 4UL, s, s_len);
                  }
                  else aprsstr_Append(s, s_len, ".", 2ul);
                  ++i;
               } while (i<=j);
                ...
                uint32_t ip, uint32_t fromport)
{
      ...
      if (typ=='y') {
         ...
         if (pc && p<(rxb_len-1)-18UL) {
            posrs41(rxb, rxb_len, p, &lat, &long0, &heig, &speed, &dir,
                &climb);
            sats = (uint32_t)(uint8_t)rxb[p+18UL];
            if (sondeaprs_verb) {
         ...
         if (pc && p<(rxb_len-1)-18UL) {
            posrs41(rxb, rxb_len, p, &lat, &long0, &heig, &speed, &dir,
                &climb);
            pc->hp = altToPres(heig); /* make hPa out of gps alt for ozone */
            /*          pc^.gpssecond:=rs41date(rxb,p+18) + (VAL(CARDINAL,
                DAYSEC-leapseconds) MOD DAYSEC); */
            pc->gpssecond = rs41date(rxb, rxb_len, p+18UL);
         }
      }
      else if (typ=='\203') {
         for (i = 0UL; i<=31UL; i++) {
            /* gps prn number range */
            /* prn number / level byte bit order may be wrong */
            if (X2C_IN(i&7UL,8,(uint8_t)(uint8_t)rxb[p+18UL+i/8UL])) {
               /* looks like sat levels start at 25 */
               j = 25UL+sats;
               if (sats<=11UL && j<=len) {
                  satsigs[sats].num = i+1UL;
                  satsigs[sats].level = (uint32_t)(uint8_t)rxb[p+j];
               }
               ++sats;
            }
         } /* end for */
         ...
         /*             WrStrLn("76 frame"); */
         if (sondeaprs_verb) {
            osi_WrStr(" Encryption frame", 18ul);
            osi_WrStr(" toa:", 6ul);
            osic_WrUINT32(sdrblock.toa/16777216UL, 1UL);
            osi_WrStr(",", 2ul);
            osic_WrUINT32(sdrblock.toa&16777215UL, 1UL);
         }
         if (pc) {
            ++pc->encrcnt;
            if (encryptmsgfn[0U] && pc->encrcnt==2UL) {
   ...
   if (pc && nameok) {
      { /* with */
         struct TOA * anonym = &pc->toas[pc->toawp];
         anonym->toa = sdrblock.toa;
         anonym->date = systime;
         anonym->udpip = ip;
         anonym->udpport = fromport;
         anonym->framenum = frameno;
      }
      pc->toawp = (pc->toawp+1UL)%11UL;
      if (((calok && lat!=0.0) && long0!=0.0) && sats>0UL) {
         res = pc->txtime;
         if (res<100000L) res -= (int32_t)frameno;
         if (X2C_STRCMP(pc->name,9u,testencrname,9u)==0) {
            ++pc->encrcnt;
            osi_WrStr("--- test encr ", 15ul);
         }
         findencrypted(pc, s, 1001ul);
         /*IF s[0]<>0C THEN WrStr("toastr:"); WrStr(s); WrStrLn("") END; */
         sondeaprs_senddata(lat, long0, heig, speed, dir, climb, 0.0,
                humidity, temperature, ozonval, pc->ozonTemp, pc->ozonPumpMA,
                ...
                psatsigs, sdrblock, leapseconds, xdatablock, s, 1001ul);
         pc->framesent = 1;
      }
   }
                ...
                uint32_t ip, uint32_t fromport)
{
   ...
   if ((((pc && nameok) && calok) && lat!=0.0) && lon!=0.0) {
      ...
      sondeaprs_senddata(lat*1.7453292519943E-2, lon*1.7453292519943E-2, alt,
                ...
                sdrblock, (int32_t)gpstimecorr, xdatablock, "", 1ul);
      pc->framesent = 1;
   }
} /* end decodem10() */
                ...
                uint32_t ip, uint32_t fromport)
{
   ...
   if ((((pc && nameok) && calok) && lat!=0.0) && long0!=0.0) {
      ...
      sondeaprs_senddata(lat*1.7453292519943E-2, long0*1.7453292519943E-2,
                ...
                5ul, "", 1ul, 0, sdrblock, leapseconds, xdatablock, "", 1ul);
      pc->framesent = 1;
   }
} /* end decodeimet() */
                ...
                int32_t * len, struct sondeaprs_SDRBLOCK * sdr)
{
   ...
   do {
      if (b[p]=='f') {
         ...
         sdr->toa = (uint32_t)(uint8_t)b[p+1UL]*16777216UL+(uint32_t)
                (uint8_t)b[p+2UL]*65536UL+(uint32_t)(uint8_t)
                b[p+3UL]*256UL+(uint32_t)(uint8_t)b[p+4UL];
         sdr->valid = 1;
      }
