...
static void rfdist(aprsdecode_pVARDAT v, char h[], uint32_t h_len)
{
   ...
   { /* with */
      ...
      if ((uint32_t)anonym->igatelen==0UL || !aprspos_posvalid(anonym->pos)
                ) return;
...
static void split(char s[], uint32_t s_len)
{
   uint32_t j;
   uint32_t i;
   j = aprsstr_Length(s, s_len);
...
static void decode(char s[], uint32_t s_len, aprsdecode_pFRAMEHIST pf0,
                 aprsdecode_pFRAMEHIST pf, aprsdecode_pVARDAT oldvar,
                uint32_t odate, char decoded, char wrapp,
                struct aprsdecode_DAT * dat)
{
   char h[512];
   char colalt;
         ...
         for (;;) {
            if (dat->tlmvalues[ret]) {
               if (ret!=6L) {
                ...
                dat->tlmvalues[ret]-1UL), 1UL, h, 512ul);
                  aprsstr_Append(s, s_len, h, 512ul);
               }
               else {
                  tn = (uint32_t)dat->tlmvalues[ret]-1UL;
   ...
   if ((wrapp && s[0UL]) && (uint8_t)pf->inport>' ') {
      aprsstr_Append(s, s_len, (char *) &pf->inport, 1u/1u);
      aprsstr_Append(s, s_len, ":", 2ul);
   }
   aprsstr_Append(s, s_len, "[", 2ul);
   h[0] = 0;
   aprstext_Apphex(h, 512ul, pf->vardat->raw, 500ul);
   if (wrapp && aprsstr_Length(h, 512ul)>120UL) split(h, 512ul);
   aprsstr_Append(s, s_len, h, 512ul);
   aprsstr_Append(s, s_len, "]", 2ul);
                ...
                char text[], uint32_t text_len, uint32_t time0)
{
   ...
   if (i>1UL) {
      ...
      f.vardat = &vardat;
      f.time0 = time0;
      decode(s, s_len, 0, &f, 0, 0UL, 1, 0, &dat);
   }
   X2C_PFREE(text);
} /* end decodelistline() */
                ...
                aprsdecode_pOPHIST op, char overwrite)
{
   ...
   else if (op && (uint32_t)op->lastinftyp<100UL) {
   ...
   *last = 0;
   if (op && pf) {
      ...
      if ((aprsdecode_lums.errorstep && pf) && (pf->nodraw&~0x40U)==0U) {
         ...
         aprsstr_Append(s, s_len, " ", 2ul);
         if (pfe==0) decode(ss, 1000ul, op->frames, pf, 0, 0UL, 1, 1, &dat);
         else {
            decode(ss, 1000ul, op->frames, pf, pfe->vardat, pfe->time0, 1, 1,
                 &dat);
         }
         aprsstr_Append(s, s_len, ss, 1000ul);
         *last = pf->next==0;
...
extern void aprstext_listop(char decoded)
{
   ...
   if (op) {
      ...
      oldt = 0UL;
      while (pf) {
         decode(s, 1000ul, op->frames, pf, oldv, oldt, decoded, 0, &dat);
                ...
                int32_t txd, int32_t level)
{
   ...
   pf.vardat = &vard;
   decode(s, 1000ul, 0, &pf, 0, 0UL, decoded, 0, &dat);
   if (decoded) {
      s1[0U] = '\012';
      s1[1U] = port;
      s1[2U] = 0;
   ...
   while (op) {
      ...
      if (typ=='N') {
         ...
         if (pf && !aprspos_posvalid(op->lastpos)) {
            while (pf->next) pf = pf->next;
            decode(s, 1000ul, 0, pf, 0, 0UL, decod, 0, &dat);
