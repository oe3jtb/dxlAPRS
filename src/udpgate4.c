...
#define udpgate4_VERS "udpgate 0.81"
...
#define udpgate4_CANTBIND "trying to bind to port "
...
-w 14501 -D /home/pi/www", 151ul);
               osi_WrStrLn("for your own startpage put file \"index.html\" in\
 (-D) root directory", 68ul);
               osi_WrStrLn("enable www server dir-list with empty file \".dir\
list \" in listable subdirectories", 82ul);
               X2C_ABORT();
            }
            if (lasth=='u') {
               osi_NextArg(h, 4096ul);
               ...
               osi_NextArg(h, 4096ul);
               i0 = 0UL;
               if (GetSec(h, 4096ul, &i0, &n)>=0L) wwwsizelimit = n*1024UL;
   ...
   while (i0<s_len-1 && s[i0]) {
      if (s[i0]=='\\' && s[i0+1UL]=='\\') {
         ...
         if (s[i0]=='z') {
            ...
            /* insert version */
            aprsstr_Append(ns, 256ul, "udpgate 0.81", 13ul);
         }
         else if (s[i0]==':') {
            /* insert file */
            fn[0U] = 0;
...
static char chkwwwfn(const char fn[], uint32_t fn_len)
{
   uint32_t i0;
   if (wwwdir[0U]==0 || fn[0UL]=='/') return 0;
   i0 = 0UL;
   for (;;) {
      ...
      if ((fn[i0]=='.' && fn[i0+1UL]=='.') && fn[i0+2UL]=='/') return 0;
      ++i0;
   }
   return 1;
} /* end chkwwwfn() */
                 ...
                 uint32_t * flen)
{
   ...
   i0 = aprsstr_Length(fn, fn_len);
   if ((i0==0UL || fn[i0-1UL]=='/') || !chkwwwfn(fn, fn_len)) {
      openfile_ret = 0;
      goto label;
   }
   aprsstr_Assign(h, 4097ul, wwwdir, 1024ul); /* wwwdir has trailing "/" */
   aprsstr_Append(h, 4097ul, fn, fn_len);
   *fd = osi_OpenNONBLOCK(h, 4097ul);
   if (*fd<0L) {
      openfile_ret = 0;
      goto label;
   }
   /*    IF (fd<0) OR (RdBin(fd,hh,1)<>1) THEN RETURN FALSE END;
   ...
   */
   *flen = (uint32_t)osic_Size(*fd);
   if (*flen>=wwwsizelimit) {
      openfile_ret = 0;
      goto label;
   }
   openfile_ret = 1;
                ...
                const char b[], uint32_t b_len)
{
   ...
   i0 = 0UL;
   do {
      if ((uint8_t)a[i0]>(uint8_t)b[i0]) return 0;
      if ((uint8_t)a[i0]<(uint8_t)b[i0]) return 1;
      ++i0;
   } while (!(((i0>a_len-1 || i0>b_len-1) || a[i0]==0) || b[i0]==0));
   return 0;
} /* end strgt() */
...
static void ap(uint32_t * j, char h[4096], const char s[],
                uint32_t s_len)
{
   uint32_t ii;
   ii = 0UL;
                ...
                uint32_t msgvia_len)
{
 ...
 style=\"text-align:center\"><h3>\015\012", 81ul);
   if (maintit) {
      ...
      Appwww(wsock, wbuf, " [udpgate 0.81] Maxusers ", 26ul);
      ...
      Appwww(wsock, wbuf, " [udpgate 0.81] http#", 22ul);
   }
...
#define udpgate4_LTAB 10
/* file len columns */

#define udpgate4_DIRLIST ".dirlist"
/* file to enable dirlist */

typedef char FN[101];


static char opendir0(WWWB wbuf, pTCPSOCK * wsock,
                const char path[], uint32_t path_len)
{
   ...
   FN pl;
   FN fns[1001];
   if (!chkwwwfn(path, path_len)) return 0;
   aprsstr_Assign(dir, 4097ul, path, path_len);
   i0 = aprsstr_Length(dir, 4097ul);
   if (i0>0UL && dir[i0-1UL]=='/') dir[i0-1UL] = 0;
   aprsstr_Assign(fn, 4097ul, wwwdir, 1024ul);
   aprsstr_Append(fn, 4097ul, dir, 4097ul);
   aprsstr_Append(fn, 4097ul, "/.dirlist", 10ul);
   if (!osi_Exists(fn, 4097ul)) return 0;
   /* dir has no file ".dirlist" */
   aprsstr_Assign(fn, 4097ul, wwwdir, 1024ul);
   aprsstr_Append(fn, 4097ul, dir, 4097ul);
   if (osi_OpenDir(fn, 4097ul, &pdir)<0L) return 0;
   aprsstr_Append(dir, 4097ul, "/", 2ul);
   dn = 0UL;
   for (;;) {
      osi_ReadDirLine(fn, 4097ul, pdir);
      if (fn[0U]==0) break;
      if (fn[0U]!='.') {
         /* not "." ".." files */
         aprsstr_Assign(fns[dn], 101ul, fn, 4097ul);
         ++dn;
         if (dn>1000UL) break;
      }
   }
   osi_CloseDir(pdir);
   /*check parent dir */
   aprsstr_Assign(parent, 4097ul, dir, 4097ul);
   ii = (int32_t)aprsstr_Length(parent, 4097ul)-2L;
   while (ii>0L && parent[ii]!='/') --ii;
   if (ii>=0L) parent[ii] = 0;
   aprsstr_Assign(fn, 4097ul, wwwdir, 1024ul);
   aprsstr_Append(fn, 4097ul, parent, 4097ul);
   aprsstr_Append(fn, 4097ul, "/.dirlist", 10ul);
   if ((parent[0U] && osi_Exists(fn, 4097ul)) && dn<1000UL) {
      fns[dn][0] = 0;
      ++dn;
   }
   /*sort files on name */
   do {
      ...
      while (i0<dn) {
         if (strgt(fns[i0], 101ul, fns[i0-1UL], 101ul)) {
            memcpy(pl,fns[i0-1UL],101u);
            memcpy(fns[i0-1UL],fns[i0],101u);
            memcpy(fns[i0],pl,101u);
            done = 0;
         }
         ++i0;
      }
   ...
   Appwww(wsock, wbuf, "<H2>Index of /", 15ul);
   Appwww(wsock, wbuf, dir, 4097ul);
   Appwww(wsock, wbuf, "</H2><pre>", 11ul);
   j = 0UL;
   while (j<dn) {
      isfile = 0;
      isdir = 0;
      ...
      if (fns[j][0]) {
         aprsstr_Assign(fn, 4097ul, wwwdir, 1024ul);
         aprsstr_Append(fn, 4097ul, dir, 4097ul);
         aprsstr_Append(fn, 4097ul, fns[j], 101ul);
         fd = osi_OpenNONBLOCK(fn, 4097ul);
         if (fd>=0L) {
            isfile = 1;
            fl = (uint32_t)osic_Size(fd);
            osic_Close(fd);
            if (fl<1000000UL) {
               aprsstr_CardToStr(fl, 1UL, h, 1001ul);
               aprsstr_Append(h, 1001ul, "B", 2ul);
            }
            else {
               aprsstr_FixToStr((float)fl*1.E-6f, 3UL, h, 1001ul);
               aprsstr_Append(h, 1001ul, "M", 2ul);
            }
            if (fl>wwwsizelimit) big = '!';
         }
         else {
            isdir = osi_OpenDir(fn, 4097ul, &pdir)>=0L;
            if (isdir) osi_CloseDir(pdir);
            aprsstr_Append(fn, 4097ul, "/.dirlist", 10ul);
            if (!osi_Exists(fn, 4097ul)) isdir = 0;
            strncpy(h,"dir",1001u);
         }
      }
      else {
         isdir = 1;
         strncpy(h,"parent dir",1001u);
      }
      if (isfile || isdir) {
         strncpy(fn,"                       ",4097u);
         i0 = aprsstr_Length(h, 1001ul);
         if (i0<=10UL) fn[10UL-i0] = 0;
         aprsstr_Append(fn, 4097ul, h, 1001ul);
         aprsstr_Append(fn, 4097ul, (char *) &big, 1u/1u);
         aprsstr_Append(fn, 4097ul, "<A HREF=\"/", 11ul);
         if (fns[j][0]==0) aprsstr_Append(fn, 4097ul, parent, 4097ul);
         else {
            aprsstr_Append(fn, 4097ul, dir, 4097ul);
            aprsstr_Append(fn, 4097ul, fns[j], 101ul);
         }
         if (isdir) aprsstr_Append(fn, 4097ul, "/", 2ul);
         aprsstr_Append(fn, 4097ul, "\">", 3ul);
         if (fns[j][0]==0) aprsstr_Append(fn, 4097ul, parent, 4097ul);
         else aprsstr_Append(fn, 4097ul, fns[j], 101ul);
         if (isdir) aprsstr_Append(fn, 4097ul, "/", 2ul);
         aprsstr_Append(fn, 4097ul, "</A>\015\012", 7ul);
         Appwww(wsock, wbuf, fn, 4097ul);
      }
      /*   3.3M  Nov 14 21:03  <A HREF="/tmp/20240815_161645.jpg"
                >20240815_161645.jpg</A> */
      /*                  146B <A HREF="/icon/170.gif">170.gif</A> */
      ++j;
   }
   return 1;
} /* end opendir() */
...
nected Stations Since Last ", 40ul, wsock->sortby);
   ...
   else if (openfile(wsock->get, 256ul, &fdw, &flen)) {
      conthead(&wsock, wbuf, (int32_t)flen);
   ...
   else if (opendir0(wbuf, &wsock, wsock->get, 256ul)) {
      Appwww(&wsock, wbuf, "</pre></body></html>\015\012", 23ul);
   }
   else {
      strncpy(wbuf,"HTTP/1.1 404\015\012",1401u);
   }
   if (wbuf[0U]) {
   ...
   *sockchain = cp;
   if (cservice=='G') {
      ...
      if (servercall[0U]) {
         ...
         aprsstr_Append(h, 512ul, "udpgate 0.81", 13ul);
         if (actfilter[0U]) {
            aprsstr_Append(h, 512ul, " filter ", 9ul);
      ...
      */
      ...
      aprsstr_Assign(h, 512ul, "# udpgate 0.81\015\012", 17ul);
      Sendtcp(cp, h);
   }
   return 1;
} /* end tcpconn() */
      ...
      if (systime!=lasttime) {
         if (listensock<0L && tcpbindport[0U]) {
            ...
            if (verb && listensock<0L) {
               osi_WrStr("trying to bind to port ", 24ul);
