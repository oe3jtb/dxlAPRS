                ...
                uint32_t ysizeh)
{
   ...
   if (aprsdecode_WITHX11) {
      ...
      { /* with */
         ...
         anonym->ximage0 = XCreateImage(dis, anonym->pvis,
                anonym->bitperpixel, 2L, 0L, (char *)xbuf016, xsizeh,
                ysizeh, 32L, 0L);
         if (anonym->ximage0==0) {
...
{
   if (aprsdecode_WITHX11) XCloseDisplay(dis);
} /* end closewin() */
...
{
   if (aprsdecode_WITHX11) {
      XPutImage(dis, mainwin.win, DefaultGC(dis, 0L), mainwin.ximage0, 0L,
                0L, 0L, 0L, xbufxsize, xbufysize);
   }
} /* end rgbtoscreen() */
...
static void checkbuttons(void)
{
   ...
   uint32_t m;
   if (aprsdecode_WITHX11) {
      if ((xosi_pulling && XQueryPointer(dis, mainwin.win, &r, &c, &x, &y,
                &wx, &wy, &m)) && ((uint32_t)m&0x300UL)==0UL) {
...
extern void xosi_xevent(void)
{
   ...
   if (aprsdecode_WITHX11) {
      while (XPending(dis)) {
         ...
         /*IF event.type<>6 THEN WrInt(event.type, 10);
                WrStrLn(" event") END; */
         switch (event.type) {
         case 4L:
            ...
            case 1UL:
               /*
                                  IF Shift THEN
                                    Pull(event.xmotion.x, event.xmotion.y,
                0);
                                    sethand;
         ...
         case 2L:
            cmd = 0;
            res = XLookupString(&event.xkey, (PChar)h, 256L, &mykey,
                &mystat);
            if (res==1L) cmd = h[0U];
            else if (res==0L) {
         ...
         case 31L:
            /*        pulloff; */
            pasteget(dis, mainwin.win, event.xselection.property, &nn,
                &pstr);
            if ((char *)pstr) {
               tmp = (int32_t)nn-1L;
               res = 0L;
               if (res<=tmp) for (;; res++) {
                  cmd = pstr[res];
                  if ((uint8_t)cmd>=' ' && (uint8_t)cmd<=(uint8_t)
                '\201') useri_keychar(cmd, 1, 0);
                  if (res==tmp) break;
               } /* end for */
