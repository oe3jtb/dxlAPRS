...
static int32_t Kbd(void)
{
   ...
   { /* with */
      ...
      anonym->c_lflag = (uint32_t)(uint16_t)((uint16_t)anonym->c_lflag&~0xAU);
   }
...
static void fix(const char s[], uint32_t s_len, uint32_t * p, float * x, char * ok0)
{
   float m;
   m = 1.0f;
   *ok0 = 0;
   *x = 0.0f;
   while ((uint8_t)s[*p]>='0' && (uint8_t)s[*p]<='9' || s[*p]=='.') {
      if (s[*p]=='.') m = 0.1f;
      else if (m==1.0f) *x =  *x*10.0f+(float)((uint32_t)(uint8_t)s[*p]-48UL);
      else {
         ...
         if (s[1U]=='m') mono = 1;
            ...
            if (ok0) {
               if (fr>2000.0f && fr<=4.294967295E+9f) {
                  tune = (uint32_t)X2C_TRUNCC(fr+0.5f,0UL,X2C_max_longcard);
               }
               else if (fr>1.0f && fr<=4294.0f) {
                  tune = (uint32_t)X2C_TRUNCC(fr*1.E+6f+0.5f,0UL,X2C_max_longcard);
               }
            ...
            if (aprsstr_StrToCard(s, 1001ul, &m) && m<256UL) {
               ...
               if (aprsstr_StrToInt(s, 1001ul, &ni)) {
                  stickparm[m].val = ni; /* stick parameter */
                  stickparm[m].ok0 = 1;
               }
            ...
            /* afc */
            osi_NextArg(s, 1001ul);
            if (!aprsstr_StrToCard(s, 1001ul, &afc) || afc>200UL) Usage(" -a <khz>", 10ul);
         }
         else if (s[1U]=='w') {
            /* if-width */
            osi_NextArg(s, 1001ul);
            if ((!aprsstr_StrToCard(s, 1001ul, &ifwidth) || ifwidth>192UL) || ifwidth<3UL) {
            ...
            /* fir len */
            osi_NextArg(s, 1001ul);
            if ((!aprsstr_StrToCard(s, 1001ul, &firl) || firl>1024UL) || firl<4UL) {
            ...
            osi_WrStrLn("Stereo UKW Radio from rtl_tcp (8 bit IQ via tcpip to 2 channel 16 bit pcm file/pipe ", 85ul);
            osi_WrStrLn(" -a <khz>       max afc <kHz> (50) or 0 for off", 48ul);
            osi_WrStrLn(" -F <Hz>        additional FIR if-filter bandwidth Hz (0)", 58ul);
            ...
            osi_WrStrLn(" -p <cmd> <value> send rtl_tcp parameter, ppm, tunergain ... -p 5 <ppm>", 72ul);
            osi_WrStrLn(" -r             RDS on", 23ul);
            osi_WrStrLn(" -s <soundfilename>  16bit 48kHz signed 2 channel sound stream/pipe with WAV-header", 84ul);
            osi_WrStrLn(" -t <url:port>  connect rtl:tcp server (127.0.0.1:1234) or <iq-file:0>", 71ul);
            osi_WrStrLn(" -v             show rssi (dB) and afc (khz)", 45ul);
            osi_WrStrLn(" -w <khz>       IF-width, less cpu 3,6,12,24,48,96,192 (mono 96, stereo 192)", 77ul);
            osi_WrStrLn(" Keys: +- (50kHz), m (toggle mono), r (toggle rds)", 51ul);
            osi_WrStrLn("example: sdrradio -f 101.2 -s /dev/stdout -t 192.168.1.1:1234 -p 5 72 -p 8 1 -r -v | aplay",
                91ul);
            X2C_ABORT();
         }
         else Usage("-h", 3ul);
...
static void showrssi(char s[], uint32_t s_len)
{
   ...
   aprsstr_FixToStr(osic_ln((rxx.rssi+1.0f)*3.0517578125E-5f)*4.342944819f, 2UL, ss, 31ul);
   aprsstr_Append(s, s_len, ss, 31ul);
...
static uint32_t getbits(const char b[], uint32_t b_len, uint32_t offset, uint32_t len)
{
   uint32_t v;
   v = 0UL;
   while (len>0UL) {
...
--    IntToStr(countrycode, 2, pitext);
  ...
  ELSIF oc="C" THEN
    ...
    IF (a>=224) & (a<=228) & (countrycode>0) THEN         (* country code *)
      ...
      countrytext[2]:=0C;
    END;
    IF (n DIV 256>=32) & (n DIV 256<127) THEN Werr(CHR(n DIV 256)) ELSE Werr(" ") END;
    IF (n MOD 256>=32) & (n MOD 256<127) THEN Werr(CHR(n MOD 256)) ELSE Werr(" ") END;
  ELSIF oc="D" THEN
    Werr(" ");
    IF (n DIV 256>=32) & (n DIV 256<127) THEN Werr(CHR(n DIV 256)) ELSE Werr(" ") END;
    IF (n MOD 256>=32) & (n MOD 256<127) THEN Werr(CHR(n MOD 256)) ELSE Werr(" ") END;
...
static char crc(const char b[], uint32_t b_len, uint32_t offset, uint32_t xor)
{
   uint32_t c;
   uint32_t i;
   c = 0UL;
...
static void frame(void)
{
   ...
   aprsstr_Append(pr, 256ul, " ", 2ul);
   aprsstr_Append(pr, 256ul, (char *)(tmp = (char)(66UL-textAB),&tmp), 1u/1u);
   aprsstr_Append(pr, 256ul, ":", 2ul);
...
{
   bitm[bitc] = bitm[bitc]+((float)(uint32_t)(b==bitb[bitc])-bitm[bitc])*0.1f;
   bitb[bitc] = b;
   if (bitm[bitc]>0.8f) {
      ++samec;
...
static void decoderds(float v)
{
   ...
   int32_t m;
   char d;
   ui = lowpass(v*osic_sin((float)rdsclk), 0.0252f, &lpi); /* not phaselinear not good for data */
   uq = lowpass(v*osic_cos((float)rdsclk), 0.0252f, &lpq);
   rdsclk = rdsclk+1.865255188222+(double)(ui*uq*2.E-7f); /* simple costas loop */
   m = 0L;
...
extern int main(int argc, char **argv)
{
   ...
   if (sdr_startsdr(url, 1001ul, port, 1001ul, 2112000UL, 192000UL, 1, 1UL)) {
      ...
      for (pc = 0UL; pc<=255UL; pc++) {
         if (stickparm[pc].ok0) sdr_setparm(pc, (uint32_t)stickparm[pc].val);
      } /* end for */
      sndw = 0UL;
      fd = -1L;
      if (soundfn[0U]) {
