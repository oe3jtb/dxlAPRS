...
/* \\!wx.txt,,t,,h,b,,w3.6,d! */
{
   ...
   if (csvget(fn, 1024ul, wms, wms_len, 0UL)) {
      ...
      if (fn[0U]) {
         ...
         if ((int32_t)w.dust10>=0L) {
            apd(ws, ws_len, "m3", 3ul, (float)w.dust10, 3UL);
         }
         if ((int32_t)w.dust2>=0L) {
            apd(ws, ws_len, "m2", 3ul, (float)w.dust2, 3UL);
         }
         if ((int32_t)w.dust1>=0L) {
            apd(ws, ws_len, "m1", 3ul, (float)w.dust1, 3UL);
         }
         if ((int32_t)w.dust01>=0L) {
...
static void sendping(aprsdecode_pTCPSOCK tp)
{
   ...
   if (aprsdecode_realtime>=tp->lastping+20UL) {
      ...
      if ((uint32_t)tp->waitpong<=1UL) ++tp->waitpong;
      strncpy(s,"#",512u);
...
static char r91(uint16_t * n, char c)
{
   ...
   *n = (uint16_t)((uint32_t)*n*91UL+((uint32_t)(uint8_t)c-33UL));
   return 1;
} /* end r91() */
...
static void GetTLM(aprsdecode_TELEMETRY v, char b[], uint32_t b_len)
{
   ...
   if (((ib>0UL && (ia-ib&1)) && ia-ib>=5UL) && ia-ib<=15UL) {
      ...
      ia = 0UL;
      if (ia<=tmp) for (;; ia++) {
         v[ia] = (uint16_t)((uint32_t)t[ia]+1UL);
         if (ia==tmp) break;
      } /* end for */
      /*    b[ib-1]:=0C; */
...
/* test for and draw item in a message */
{
   ...
   if ((aprsdecode_Decode(s, 1000ul,
      ...
      aprsdecode_drawbeacon(h, 1000ul, 1);
      *pos = dat.pos;
      aprsstr_Assign(name, name_len, dat.symcall, 9ul);
      *del = dat.objkill=='1';
   }
                ...
                uint32_t rawbuf_len)
{
   ...
   if (op) {
      ...
      */
      ...
      aprsdecode_tracenew.follow = (char)((uint32_t)doset/2UL&1);
   ...
   if (aprsdecode_Decode(waypoint->vardat->raw, 500ul,
      ...
      if (ip) {
         ...
         for (;;) {
            ...
            ojp = jp;
            jp = table[jp].next;
            if ((uint32_t)jp>4095UL || (uint32_t)ojp<=(uint32_t)
                jp==((uint32_t)ojp<*wridx==(uint32_t)jp<*wridx)) {
                ...
                const struct aprsdecode_DAT dat)
{
   ...
   if (dat.sym=='_') {
      ...
      if (dat.speed<500UL) {
         op->lastkmh = (short)X2C_TRUNCI((float)dat.speed*1.609f+0.5f,
                -32768,32767);
         if ((uint32_t)op->lastinftyp<100UL) op->lastinftyp = 100U;
         op->temptime = aprsdecode_systime;
      }
      else op->lastkmh = 0;
   }
   ...
   else if ((uint32_t)op->lastinftyp<100UL) op->lastinftyp = 0U;
   if ((uint32_t)op->lastinftyp<100UL) {
                ...
                uint32_t oldobj)
{
   ...
   */
   ...
   while (op) {
      ...
      else if ((0x2U & op->drawhints) && (uint32_t)op->lastinftyp<100UL) {
            ...
            if (s[0UL]) {
               if (aprsdecode_Decode(s, 512ul, &dat)>=0L) {
                  ...
                  if (X2C_CAP(port)=='M') {
                     ...
                     aprsdecode_drawbeacon(s, 512ul, 0);
                  }
...
extern void aprsdecode_tcpjobs(void)
{
   ...
   for (i = 0L; i<=3L; i++) {
      { /* with */
         ...
         isnew = useri_isupdated((uint8_t)(39UL+(uint32_t)i));
         if (isnew) {
            anonym0->lastudprx = 0UL;
                /* no delay if updated after port in use */
         }
         if (useri_configon((uint8_t)(39UL+(uint32_t)i)) && !isnew) {
            /*      IF CAST(INTEGER,fd)<0 THEN */
            if ((int32_t)anonym0->fd<0L && labs((int32_t)
                (anonym0->lastudprx-aprsdecode_realtime))>=60L) {
               ok0 = 0;
...
Rx)Port or in Use", 44ul);
               ...
               /*          IF NOT ok THEN configbool(VAL(CONFSET,
                ORD(fUDP1)+VAL(CARDINAL,i)), FALSE) END;
                (* close udp port if get no bind *) */
               if (!ok0) {
                  anonym0->lastudprx = aprsdecode_realtime;
                /* start timer for next try */
               }
            }
         }
   ...
   if (local) res = 0L;
      ...
      if (((res>=0L && useri_configon(useri_fALLOWGATE)) && !tabchk)
         ...
         if (aprsstr_InStr(s, 21ul,
            ...
            while (tp) {
               if (tp->lastping>0UL && ((uint32_t)
                tp->waitpong==1UL && tp->lastping+10UL<aprsdecode_realtime || (uint32_t)
                tp->waitpong>1UL)) {
                  if (tp->lastpong>0UL) {
                ...
                char foreign)
{
   ...
   if (foreign || getbeaconparm(b, 512ul, &time0, &port)) {
      if (!foreign) beaconmacros(b, 512ul, "", 1ul, "", 1ul, 1);
      if (b[0UL]) {
         memset((char *) &modeminfo,(char)0,sizeof(struct UDPSET));
         if (aprsdecode_lums.logmode) {
